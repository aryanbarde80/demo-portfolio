...
export default function EducationNode() {
  ...
  return (
    <OSWindow title="Education" icon={<GraduationCap size={16} className="text-[#818cf8]" />}>
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center pb-2 border-b border-[#818cf8]/20">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] sm:text-[10px] mono text-gray-500">Academic Record</span>
          </div>
        ...
        {education.map((edu, i) => {
          ...
          return (
            <div 
              key={i} 
              className="group relative p-4 border border-[#818cf8]/20 bg-gradient-to-r from-[#030712] to-transparent rounded-lg hover:border-[#818cf8]/60 transition-all duration-300 overflow-hidden"
            >
              {/* Animated Background Glow */}
              <div className="absolute inset-0 bg-gradient-to-r from-[#818cf8]/0 via-[#818cf8]/5 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
              
              {/* Left Accent Border */}
              <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-[#818cf8] via-[#f472b6] to-[#818cf8] opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              
              {/* Header Section */}
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
                <div className="flex items-start gap-2 flex-1">
                  <IconComponent size={16} className="text-[#fb923c] mt-0.5 flex-shrink-0" />
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      <p className="text-[#f472b6] mono text-[10px] sm:text-xs uppercase flex items-center gap-1">
                        <span>{edu.inst}</span>
                      </p>
                      {edu.location && (
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3 pl-6 sm:pl-7">
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-[11px] sm:text-xs font-mono text-[#818cf8] bg-[#818cf8]/5 px-2 py-1 rounded flex items-center gap-1">
                    <Award size={10} />
                    {edu.score}
                  </span>
                ...
                <div className="pl-6 sm:pl-7 mt-2">
                  <div className="flex flex-wrap gap-1.5">
                    {edu.highlights.map((highlight, idx) => (
                      <span 
                        key={idx} 
                        className="text-[8px] sm:text-[9px] mono px-1.5 py-0.5 bg-gray-900/80 border border-gray-800 rounded text-gray-400 group-hover:border-[#818cf8]/30 group-hover:text-[#818cf8] transition-all"
                      >
                        {highlight}
                      </span>
                    ))}
        ...
        <div className="mt-3 pt-3 border-t border-[#818cf8]/20 flex flex-wrap justify-between items-center gap-2 text-[8px] sm:text-[9px] mono">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1">
              ...
              <span className="text-gray-500">Current CGPA: 8.14</span>
            </span>
            <span className="flex items-center gap-1">
              <Award size={10} className="text-[#fb923c]" />
              <span className="text-gray-500">Highest: 91%</span>
            </span>
          </div>
