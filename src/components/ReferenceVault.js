      ...
      <div className="space-y-4">
        ...
        <div className="relative overflow-hidden p-4 border border-[#818cf8]/30 bg-gradient-to-r from-[#818cf8]/5 to-transparent rounded-xl">
          <div className="absolute top-0 right-0 w-24 h-24 opacity-5">
            <Fingerprint size={96} className="text-[#818cf8]" />
          </div>
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-[#818cf8]/10 border border-[#818cf8]/30 animate-pulse">
                <LockKeyhole size={16} className="text-[#818cf8]" />
              </div>
              <div>
                <p className="text-[10px] sm:text-[11px] mono text-[#fb923c] uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="text-right">
              <div className="flex items-center gap-1">
                <BadgeCheck size={12} className="text-[#818cf8]" />
                <span className="text-[9px] mono text-[#818cf8]">TRUST_SCORE</span>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {credentials.map((cred, idx) => (
            <div 
              key={idx} 
              className="group p-3 border border-gray-800 hover:border-[#818cf8]/40 rounded-lg bg-[#030712]/40 hover:bg-[#818cf8]/5 transition-all duration-300 text-center"
            >
              <div className="w-8 h-8 mx-auto mb-2 rounded-full bg-[#818cf8]/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                {cred.type === "ACADEMIC" && <Shield size={14} className="text-[#818cf8]" />}
                {cred.type === "INDUSTRY" && <CircuitBoard size={14} className="text-[#fb923c]" />}
                {cred.type === "TECHNICAL" && <Cpu size={14} className="text-[#f472b6]" />}
              </div>
              <p className="text-[10px] font-bold text-gray-300">{cred.type}</p>
              <p className="text-[8px] mono text-[#818cf8] mt-1">{cred.status}</p>
        ...
        <div className="space-y-2">
          <h4 className="text-[9px] sm:text-[10px] mono text-[#fb923c] font-bold flex items-center gap-2">
            <Scan size={10} /> AUDIT_TRAIL.log
          </h4>
          <div className="space-y-1.5 max-h-[180px] overflow-y-auto pr-1">
            {auditLog.map((log, idx) => (
              <div key={idx} className="flex items-center justify-between text-[8px] sm:text-[9px] mono border-l-2 border-[#818cf8]/30 pl-2 py-1 hover:border-[#818cf8] transition-all">
                <span className="text-gray-400">{log.action}</span>
                <div className="flex gap-3">
                  <span className="text-gray-600">{log.timestamp}</span>
                  <span className="text-[#818cf8]/60">{log.hash}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Verification Status */}
        <div className="flex flex-wrap justify-between items-center gap-2 pt-2 border-t border-[#818cf8]/20 text-[8px] sm:text-[9px] mono">
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
            <span className="text-gray-500">VAULT_STATUS: ACTIVE</span>
          </div>
          <div className="flex items-center gap-2">
            <Key size={8} className="text-[#fb923c]" />
